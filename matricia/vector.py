...
class Vector:
    ...
    _name_map = {"x": 0, "y": 1, "z": 2, "w": 3, "a": 0, "b": 1, "c": 2, "d": 3}
        ...
        if kwargs:  # handles Vector(x=1, y=2, z=3)
            for key, value in kwargs.items():
                if key in Vector._name_map:
                    ...
                    while (
                        len(temp_list) <= idx
                    ):  # handles Vector(z=2) by creating temp_list = [0,0,2]
                        temp_list.append(0)
    ...
    def same_dimensions(*vectors: Vector) -> bool:
        ...
        for i, v in enumerate(vectors):
            if not isinstance(v, Vector):
                raise TypeError(
                    f"Argument at index {i} is {type(v).__name__}, not a Vector."
                )
    ...
    def _get_fractions(self):
        ...
        return [
            (f.numerator)
            if (
                f := Fraction(x).limit_denominator(Vector.DISPLAY_PRECISION)
            ).denominator
            == 1
            else (f)
    ...
    def __setattr__(self, name, value):
        ...
        if not getattr(self, "mutable", False):
            raise AttributeError(
                "This vector is immutable. Use mutable=True to modify."
            )
    ...
    def __getattr__(self, name):
        if name in Vector._name_map:
            ...
            try:
                ...
                raise IndexError(
                    f"The Vector doesn't have a value at the component '{name}'."
                )
    ...
    def __add__(self, other):
        ...
        if isinstance(other, (int, float)):
            return Vector(
                [x + other for x in self.components],
                mutable=(self.mutable and other.mutable),
            )
    ...
    def __imul__(self, other):
        ...
        if isinstance(other, Vector):
            raise TypeError(
                "Cannot use *= for dot products because the result is a scalar."
            )
    ...
    def cross(self, other: Vector) -> Vector:
        ...
        """
        if (
            len(self.components) != 3
            or not isinstance(other, Vector)
            or len(other.components) != 3
        ):
    ...
    def normalize(self) -> Vector:
        ...
        return Vector(
            [x / magnitude_value for x in self.components], mutable=self.mutable
        )
    ...
    @staticmethod
    def are_parallel(
        v1: Vector, v2: Vector, allow_antiparallel: bool = True, tol: float = EPSILON
    ) -> bool:
    ...
    @staticmethod
    def are_all_parallel(
        *vectors: Vector, allow_antiparallel: bool = True, tol: float = EPSILON
    ) -> bool:
        if len(vectors) < 2:
            return True
        ...
        for v in vectors[1:]:
            if not Vector.are_parallel(
                anchor, v, allow_antiparallel=allow_antiparallel, tol=tol
            ):
                return False
    ...
    def mean(*vectors: Vector) -> Vector:
        ...
        if not Vector.same_dimensions(vectors):
            raise ValueError(
                f"Cannot calculate the mean vector. Vectors are not all of the same dimensions."
            )
    ...
    def angular_spread(*vectors: Vector, in_degrees: bool = False) -> float:
        ...
        sum_of_angles = sum(
            Vector.angle_between(v, mean_vector, in_degrees=in_degrees) for v in vectors
        )
    ...
    def get_linear_combination(self, *vectors):
        ...
        """
        from matricia.matrix import Matrix
    ...
    def matricization(
        self, row_num: int, col_num: int, by_column: bool = False, mutable: bool = False
    ):
        if len(self.components) != row_num * col_num:
            raise ValueError(
                f"Cannot reshape a vector of size {len(self.components)} into a {row_num}x{col_num} matrix."
            )
        ...
        if not by_column:
            ...
            return Matrix(
                [
                    Vector(self.components[i * col_num : (i + 1) * col_num])
            ...
            return Matrix(
                [
                    Vector([self.components[r + c * row_num] for c in range(col_num)])
