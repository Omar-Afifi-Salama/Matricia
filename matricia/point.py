...
class Point:
        ...
        if kwargs:  # handles Point(x=1, y=2, z=3)
            for key, value in kwargs.items():
                if key in Point._name_map:
                    ...
                    while (
                        len(temp_list) <= idx
                    ):  # handles Point(z=2) by creating temp_list = [0,0,2]
                        temp_list.append(0)
    ...
    def _get_fractions(self):
        ...
        return [
            (f.numerator)
            if (f := Fraction(x).limit_denominator(Point.DISPLAY_PRECISION)).denominator
            == 1
            else (f)
    ...
    def __getattr__(self, name):
        if name in Point._name_map:
            ...
            try:
                ...
                raise IndexError(
                    f"The Point doesn't have a value at the component '{name}'."
                )
    ...
    def distance_to(self, other):
        ...
        return math.sqrt(
            sum((b - a) ** 2 for a, b in zip(self.components, other.components))
        )
    ...
    def centroid(*points):
        ...
        return Point(
            [sum(c) / num_points for c in zip(*(p.components for p in points))],
            mutable=all(p.mutable for p in points),
        )
    ...
    def are_collinear(*points: Point, tol=EPSILON):
        ...
        for i, p in enumerate(points):
            if not isinstance(p, Point):
                raise TypeError(
                    f"Argument at index {i} is {type(p).__name__}, not a Point."
                )

            if not len(anchor_point) == len(p):
                raise TypeError(
                    f"Argument at index {i} is of {len(p)} not {len(anchor_point)}"
                )
