...
class Matrix:
    ...
    def __init__(self, *args, mutable=False, **kwargs):
        ...
        if len(args) == 1 and hasattr(args[0], '__iter__') and not isinstance(args[0], (list, tuple, Vector, str)):
            # "Drain" the generator into a list
            # args[0] becomes [item1, item2, ...]
            args[0] = list(args[0])
            
            # Now, check if the items INSIDE were also generators
            if len(args[0]) > 0 and hasattr(args[0][0], '__iter__') and not isinstance(args[0][0], (list, tuple, Vector, str)):
        ...
        if len(args) == 1:
            ...
            c = kwargs.get('cols') or kwargs.get('col') or kwargs.get('columns') or kwargs.get('c') or kwargs.get('n')
        ...
        self.data = [
            row if isinstance(row, Vector) and row.mutable == self.mutable
    ...
    def get_column(self, idx):
        ...
        raise IndexError(f"Matrix column index {idx} out of range (0 to {self.col_num - 1}).")
    ...
    def __setitem__(self, key, value):
        if not self.mutable:
            raise AttributeError("This Matrix is immutable. Set mutable=True to modify.")
    ...
    def __str__(self):
        ...
        def format_val(val):
            ...
            if getattr(Matrix, 'FRACTIONS', False):
                f = Fraction(val).limit_denominator(getattr(Vector, 'DISPLAY_PRECISION', 1000)) # TODO think about if it should depend on the Matrix class attr or the Vector # i think we should use the lowest
    ...
    def __bool__(self): # TODO implement the option to choose between strict and lenient zero
    ...
    def copy(self, mutable = None):
        ...
        return Matrix([Vector(list(row.components), mutable=mutable) for row in self.data], mutable=mutable)
    ...
    def row_echelon_form(self, tol=EPSILON) -> Matrix:
        ...
        for col_num in range(m.col_num):
            ...
            while target_row < m.row_num and math.isclose(m[target_row][col_num], 0.0, abs_tol=tol):
                target_row += 1
            ...
            if target_row != pivot_row:
                m.data[pivot_row], m.data[target_row] = m.data[target_row], m.data[pivot_row]
    ...
    def reduced_row_echelon_form(self) -> Matrix:
        ...
        for col_num in range(m.col_num):
            ...
            # Find the pivot
            while target_row < m.row_num and math.isclose(m[target_row][col_num], 0.0, abs_tol=1e-9):
                target_row += 1
    ...
    def generate_identity_matrix(size, mutable=False):
        return Matrix([[1 if i == j else 0 for j in range(size)] for i in range(size)], mutable=mutable)
    ...
    def inverse_using_adjoint(self):
        ...
        return Matrix(
            [
                [
                    (1 / det_A) * ((-1) ** (i + j)) * Matrix(
                        [[self.data[r][c] for c in range(self.col_num) if c != i] for r in range(self.row_num) if r != j]).determinant()
                        for j in range(self.col_num)
                ]
                for i in range(self.row_num)
            ], mutable=self.mutable)
    ...
    @staticmethod
    def generate_random_matrix(rows, cols, mutable = False, lower_limit = -100, upper_limit = 100):
        import random
    ...
    def extract_left(self, split_index: int) -> 'Matrix':
        ...
        new_data = [
            Vector(row.components[:split_index])
            for row in self.data
        ]
        return Matrix(new_data, mutable=self.mutable)
